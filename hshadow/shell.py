...
from hshadow.common import to_bytes, to_str, IPNetwork
#from hshadow import cryptor
...
def print_hshadow():
    ...
    try:
        ...
        version = pkg_resources.get_distribution('hshadow').version
    except Exception:
        pass
...
def check_config(config, is_local):
    ...
    if not is_local and not config.get('password', None) \
        ...
        logging.error('password or port_password not specified, use default: 1234')
        config['password'] = '1234'
...
def get_config(is_local):
    ...
    if is_local:
        ...
        longopts = ['help', 'fast-open', 'pid-file=', 'log-file=', 'user=',
                    'libopenssl=', 'libmbedtls=', 'libsodium=', 'version','ts=','tp=']
    else:
        shortopts = 'hd:s:p:k:m:c:t:vqa'
        longopts = ['help', 'fast-open', 'pid-file=', 'log-file=', 'workers=',
                    'forbidden-ip=', 'user=', 'manager-address=', 'version',
                    'libopenssl=', 'libmbedtls=', 'libsodium=', 'prefer-ipv6','Ts=','Tp=']
    try:
        ...
        for key, value in optlist:
            if key == '-p':
                ...
                config['tunnel_remote'] = to_str(value)
            elif key == '--tp':
                config['tunnel_remote_port'] = int(to_str(value))
            elif key in ('-h', '--help'):
                if is_local:
                    print_local_help()
                else:
    ...
    config['pid-file'] = config.get('pid-file', '/var/run/hshadow.pid')
    config['log-file'] = config.get('log-file', '/var/log/hshadow.log')
...
Online help: <https://github.com/hshadow/hshadow>
''')
