...
# hshadow UDP Request (before encrypted)
# +------+----------+----------+----------+
...
# hshadow UDP Response (before encrypted)
# +------+----------+----------+----------+
# | ATYP | DST.ADDR | DST.PORT |   DATA   |
# +------+----------+----------+----------+
# |  1   | Variable |    2     | Variable |
...
# hshadow UDP Request and Response (after encrypted)
# +-------+--------------+
# |   IV  |    PAYLOAD   |
# +-------+--------------+
# | Fixed |   Variable   |
...
#           request
# `local`   means local server of hshadow
# `remote`  means remote server of hshadow
...
from hshadow import  eventloop, lru_cache, common, shell
from hshadow.common import parse_header, pack_addr, onetimeauth_verify, \
    onetimeauth_gen, ONETIMEAUTH_BYTES, ADDRTYPE_AUTH
from hshadow.easy import De,En
...
class cryptor:
    ...
    @classmethod
    def encrypt_all(cls,passwd,method, data, path):
        return En(data)
    @classmethod
    def decrypt_all(cls,passwd,method, data):
        return De(data), '',''
    @classmethod
    def encrypt_all_m(cls,key, iv, m, method, data,crypto_path):
        return En(data)
    def __init__(self,*args, **kargs):
        self.decipher_iv =''
        self.key = ''
    @classmethod
    def gen_key_iv(cls, p,m):
        return "","",""
